...
def data_generate():
    ...
    pathlist_txt = glob.glob(os.path.join(config['Train']['DataPath'],"*.txt"))
    ...
    for file in pathlist_txt:
        ...
        with open('./{dir}/{fname}.csv'.format(dir = tar_dir, fname = os.path.basename(file).split('.')[0]), 'w', newline = '') as csvfile:
